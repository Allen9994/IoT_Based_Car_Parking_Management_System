...
int card_status; //Holds the value rerieved from NODEMCU : 1/2/3. Used to control the gate and for appropriate comment for the LCD display.
...
void setup(){ //Setting up the components, variables and basic functions. 
  ...
  readSensor(); //Reads value from IR sensors for the first time. Initialises the variable slot.
...
void getCardStatus() // To convert the 2-bit data from NodeMCU to a decimal value. 
{
  delay(100);
  pin1 = digitalRead(22);
  pin2 = digitalRead(24);
  if(pin1 == LOW && pin2 == LOW) //No response.
  {
    if(digitalRead(ir_enter) == 0 || digitalRead(ir_back) == 0) getCardStatus();//To recurse only if the car is still waiting in the queue.
    else flag1 = LOW;  flag2 = LOW;
  }
  else if(pin1 == LOW  && pin2 == HIGH)  card_status = 1; //Insufficient balance from the card.
  else if(pin1 == HIGH && pin2 == LOW)   card_status = 2; //Card not recognized.
  else if(pin1 == HIGH && pin2 == HIGH)  card_status = 3; //Allow the entry of the user.
}

void nearestParkingSlot() //To find the nearest parking slot.
{ //Compares with empty slot order in ascending order and stores the value accordingly.
       if(S1 == 0)  nearest_slot = 1;
  else if(S2 == 0)  nearest_slot = 2;
  else if(S3 == 0)  nearest_slot = 3;
  else if(S4 == 0)  nearest_slot = 4;
  else if(S5 == 0)  nearest_slot = 5;
  else if(S6 == 0)  nearest_slot = 6;
      lcd.clear();
      lcd.setCursor(0,1);
      lcd.print("Kindly Park your car");
      lcd.setCursor(0,2); 
      lcd.print("at Slot ");lcd.print(nearest_slot);    //Asks the driver to park their car in nearest slot.
      delay(2000);  
      lcd.clear();
}
...
void loop(){
  ...
  readSensor(); //Read the values from the IR sensors in real time. Keeps the slot information updated. 
...
{   // the car is attempting to enter.
  ...
  { // vacanat slot and no congestion.
    ...
    { 
      card_status = 0;  //Data from NodeMCU.
      ...
      getCardStatus();  //Calls the checking function to accept value from NodeMCU
      lcd.clear();
      if(card_status == 3)  //To allow the car to enter the parking lot.
      {
          delay(100);
          Serial.println("ACCESS");
          myservo.write(180);  //Opens the gate my signaling the motor to rotate 180D.
          slot -= 1;  card_status = 0; // Decrements the slot count by 1.
          nearestParkingSlot(); //Calls Near function to find and display nearest slot
       }
      else if(card_status == 1)  //Insufficient balance detected.
      {
...
void readSensor(){ //Reads value from the IR sensors and updates the slot variables.
